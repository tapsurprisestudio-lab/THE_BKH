...
const state = {
    ...
    cart: JSON.parse(sessionStorage.getItem('bkh_cart')) || [],
...
const langData = {
    ar: {
        ...
        review_text_placeholder: 'اكتبي تجربتك...',
        ...
        toast_cart_cleared: 'تم مسح السلة',
...
// ==========================================
// CART - Using sessionStorage (not localStorage)
// ==========================================
function initCart() {
...
// FIXED: Use sessionStorage instead of localStorage for cart
function saveCart() {
    sessionStorage.setItem('bkh_cart', JSON.stringify(state.cart));
}
...
function copyOrder() {
    ...
    navigator.clipboard.writeText(message).then(() => {
        ...
        // Offer to clear cart after copy
        setTimeout(() => {
            if (confirm(lang === 'ar' ? 'هل تريدين مسح السلة؟' : 'Clear cart?')) {
                clearCart();
                showToast(langData[lang].toast_cart_cleared);
            }
        }, 1000);
    });
}
...
// ==========================================
// CHATBOT - FIXED with proper state machine
// ==========================================
function initChatbot() {
    ...
    // Use click on toggle to open/close and reset chat
    toggle.addEventListener('click', (e) => {
        e.stopPropagation();
        if (panel.classList.contains('active')) {
            // Closing - reset chat state
            panel.classList.remove('active');
            state.chatbotActive = false;
        } else {
            // Opening - start fresh
            state.chatbotActive = true;
            showChatbotGreeting();
    ...
    tellMeBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        state.chatbotActive = true;
    ...
    // Use event delegation for chatbot buttons
    $('#chatbotOptions').addEventListener('click', handleChatbotClick);

    // Click outside to close and reset
    document.addEventListener('click', (e) => {
        if (!e.target.closest('#chatbotPanel') && !e.target.closest('#chatbotToggle')) {
...
function handleChatbotClick(e) {
    ...
    // Determine which flow based on action
    if (action === 'chat_gift') {
        showChatbotFlow('gift');
    } else if (action === 'chat_set') {
        showChatbotFlow('set');
    } else if (action === 'chat_ring') {
        showChatbotFlow('ring');
    } else if (action === 'chat_pricing') {
        showChatbotFlow('pricing');
    } else if (action === 'chat_howto') {
        showChatbotFlow('howto');
    } else if (action.startsWith('gift-')) {
        const price = parseInt(action.split('-')[1]);
        showGiftSuggestion(price);
        ...
        openIG();
    } else if (action.startsWith('ring-select-')) {
...
function showChatbotGreeting() {
    ...
    options.innerHTML = `
        <button class="chatbot-btn" data-action="chat_gift">${langData[lang].chat_gift}</button>
        <button class="chatbot-btn" data-action="chat_set">${langData[lang].chat_set}</button>
        <button class="chatbot-btn" data-action="chat_ring">${langData[lang].chat_ring}</button>
        <button class="chatbot-btn" data-action="chat_pricing">${langData[lang].chat_pricing}</button>
        <button class="chatbot-btn" data-action="chat_howto">${langData[lang].chat_howto}</button>
    `;
}
...
function openChatbotFlow(flow) {
    showChatbotFlow(flow);
}

function showChatbotFlow(flow) {
    const lang = state.language;
    const messages = $('#chatbotMessages');
    const options = $('#chatbotOptions');
    ...
    if (flow === 'gift') {
        ...
        options.innerHTML = `
            <button class="chatbot-btn" data-action="gift-20000">${langData[lang].chat_20k}</button>
            <button class="chatbot-btn" data-action="gift-70000">${langData[lang].chat_70k}</button>
            <button class="chatbot-btn" data-action="gift-170000">${langData[lang].chat_170k}</button>
            <button class="chatbot-btn" data-action="gift-200000">${langData[lang].chat_200k}</button>
        `;
    } else if (flow === 'set') {
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_set_info}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_eid_promo}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_crystal}</div>`;
        
        showEndOptions();
    } else if (flow === 'ring') {
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_ring_info}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_ring_20k}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_ring_30k}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_which_style}</div>`;
        
        options.innerHTML = `
            <button class="chatbot-btn" data-action="ring-select-1">${langData[lang].chat_add}</button>
            <button class="chatbot-btn" data-action="ring-select-3">${langData[lang].chat_add}</button>
        `;
        
        showEndOptions(options);
    } else if (flow === 'pricing') {
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_pricing_info}</div>`;
        messages.innerHTML += `<div class="chatbot-msg">• Rings: 20,000 - 30,000 DZD</div>`;
        messages.innerHTML += `<div class="chatbot-msg">• Bracelets: 70,000 DZD</div>`;
        messages.innerHTML += `<div class="chatbot-msg">• Necklaces: 70,000 DZD</div>`;
        messages.innerHTML += `<div class="chatbot-msg">• Sets: 170,000 - 200,000 DZD</div>`;
        messages.innerHTML += `<div class="chatbot-msg">• Earrings: 170,000 DZD</div>`;
        messages.innerHTML += `<div class="chatbot-msg">${langData[lang].chat_delivery}</div>`;
...
function showGiftSuggestion(budget) {
    const lang = state.language;
    const messages = $('#chatbotMessages');
    const options = $('#chatbotOptions');
...
function showEndOptions(container = null) {
...
function initKeyboard() {
    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape') {
            // Priority: modal > cart > menu > chatbot
